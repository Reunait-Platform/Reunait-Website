					...
					<div className="rounded-2xl border border-border bg-card p-6 sm:p-8 shadow-lg">
						...
						{step === "request" ? (
							<form onSubmit={requestCode} className="space-y-5">
								...
								<Button
									type="submit"
									className="w-full h-11 cursor-pointer text-base font-medium"
									disabled={loading || isProcessing || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || !isLoaded}
								>
							...
							<form onSubmit={verifyAndReset} className="space-y-6">
									...
									<>
										<div className="space-y-3">
											...
											{password.length > 0 && password.length < 7 && (
												<p id="password-help" className="text-xs text-destructive mt-1">
								...
								<div className="flex items-center justify-between pt-1">
									<Link href="/sign-in" className="text-sm text-muted-foreground hover:text-foreground cursor-pointer">
										Back to sign in
									</Link>
									<Button
										type="submit"
										className="h-11 cursor-pointer text-base font-medium"
										disabled={loading || isProcessing || code.replace(/\D/g, "").length !== 6 || password.length < 7 || password !== confirmPassword || !isLoaded}
									>
										Reset password
									</Button>
								</div>
								<div className="flex items-center justify-center">
									{resendCooldown > 0 ? (
										<span className="text-sm text-muted-foreground text-center">
											Resend code in {String(Math.floor(resendCooldown / 60)).padStart(1, "0")}:{String(resendCooldown % 60).padStart(2, "0")}
										</span>
									) : (
										<Button
											type="button"
											variant="ghost"
											className="h-auto p-0 text-sm text-primary cursor-pointer font-medium"
											onClick={resendCode}
											disabled={resendLoading || isProcessing}
										>
